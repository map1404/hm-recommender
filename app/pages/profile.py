...
def render():
    ...
    if cold_start:
        st.markdown("<div style='text-align: center; padding: 60px 20px;'><h2 style='font-weight: 300;'>Welcome</h2><p style='color: #666;'>Browsing in Guest Mode.</p></div>", unsafe_allow_html=True)
        return

    if not customer_id:
        return

    st.markdown("<h2 style='font-weight: 300; border-bottom: 1px solid #EAEAEA; padding-bottom: 15px; margin-bottom: 30px;'>PROFILE ANALYSIS</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<h4 style='font-size: 11px; letter-spacing: 1px; color: #888; margin-bottom: 10px;'>STYLE ARCHETYPE</h4>", unsafe_allow_html=True)
    cols = st.columns([1, 1, 1, 3])
    for i, tag in enumerate(["SPORTY", "MINIMAL", "FUNCTIONAL"]):
        cols[i].markdown(f"<div style='border:1px solid #222; padding:4px 0; font-size:10px; text-align:center; font-weight:bold;'>{tag}</div>", unsafe_allow_html=True)

    st.markdown("<h4 style='font-size: 11px; letter-spacing: 1px; color: #888; margin-top:25px; margin-bottom: 15px;'>PERSONALIZED INSIGHTS</h4>", unsafe_allow_html=True)
    ...
    # ── Category Chart ──────────────────────────────────────────────────
    st.markdown("<h4 style='font-size: 11px; letter-spacing: 1px; color: #888; margin-top:40px; margin-bottom: 20px;'>TOP CATEGORIES</h4>", unsafe_allow_html=True)
    ...
    try:
        ...
        if not history.empty:
            cats = history["article_id"].map(articles["product_type_name"].to_dict()).value_counts().head(6).reset_index()
            cats.columns = ["Category", "Count"]
            
            fig = px.bar(cats, x="Count", y="Category", orientation="h", color_discrete_sequence=['#E50010'])
            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                font=dict(color='#222222', family='Noto Sans'),
                xaxis=dict(showgrid=False, visible=False), 
                yaxis=dict(showgrid=False, title=None, categoryorder="total ascending"),
                margin=dict(l=0, r=0, t=0, b=0), height=300
            )
            fig.update_traces(texttemplate='%{x}', textposition='outside', textfont_color='#222222')
