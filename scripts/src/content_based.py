...
cosine similarity between a user preference vector and all article embeddings.
...
def user_vector(customer_id: str, article_embeddings: np.ndarray) -> np.ndarray:
    """
    Compute a user's taste vector as the purchase-count-weighted mean
    of the embeddings of articles they have bought.
    """
...
def cosine_scores(user_vec: np.ndarray, article_embeddings: np.ndarray) -> np.ndarray:
    ...
    return article_embeddings @ user_vec
